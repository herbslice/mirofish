...
class SimulationConfigGenerator:
    ...
    def __init__(
        ...
        import logging
        _logger = logging.getLogger("mirofish.config_generator")
        _logger.debug(f"Creating OpenAI-compatible client: base_url={self.base_url}, model={self.model_name}, api_key_configured={bool(self.api_key)}")
        print(f"[CONFIG_GEN DEBUG] base_url={self.base_url} model={self.model_name}", flush=True)
    ...
    def _call_llm_with_retry(self, prompt: str, system_prompt: str) -> Dict[str, Any]:
        ...
        for attempt in range(max_attempts):
            try:
                print(f"[LLM_CALL] attempt={attempt+1} model={self.model_name} base_url={self.base_url}", flush=True)
                ...
                print(f"[LLM_RESP] finish={finish_reason} content_len={len(content) if content else 0} content_start={content[:80] if content else 'NONE'}", flush=True)
